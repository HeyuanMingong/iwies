...
class EnvWorker(mp.Process):
    ...
    def empty_step(self):
        observation = np.zeros(self.env.unwrapped.observation_space.shape,
                               dtype=np.float32)
    ...
    def run(self):
        while True:
            ...
            elif command == 'reset_oracle':
                self.env.unwrapped.reset_oracle(data)
                self.remote.send(True)
            elif command == 'close':
                self.remote.close()
                break
...
class SubprocVecEnv(gym.Env):
    ...
    def reset_oracle(self, oracles):
        for remote, oracle in zip(self.remotes, oracles):
            remote.send(('reset_oracle', oracle))
        return np.stack([remote.recv() for remote in self.remotes])
